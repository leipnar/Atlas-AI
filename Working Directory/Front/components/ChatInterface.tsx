...
const ChatBubble: React.FC<{ message: ChatMessage; onFeedback: (messageId: string, feedback: 'good' | 'bad') => void; }> = ({ message, onFeedback }) => {
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, isLoading, onSendMessage, onClearChat, onFeedback }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex-shrink-0 p-2 sm:p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50/60">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">{t('chatConversationTitle')}</h2>
          {isLoading ? (
            <p className="text-xs text-blue-600 animate-pulse">{t('chatTyping')}</p>
          ) : (
      ...
      <div className="flex-grow p-2 md:p-6 space-y-6 overflow-y-auto flex flex-col">
        {messages.map((msg) => (
      ...
      <div className="p-4 border-t border-gray-200 bg-white/80">
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            className="w-full bg-gray-100 border border-gray-300 rounded-lg py-3 ps-4 pe-14 text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 resize-none"
            rows={1}
            disabled={isLoading}
          />
          <button
